...
int convert_bmp_data(U32 *bmp,U32 *bmp_buffer, U32 bpp,BITMAPINFOHEADER *info_header)
{
	...
	else //24bpp
	{
		...
		memcpy((char*)bmp_buffer,(char*)bmp,BMP_HEIGHT*BMP_WIDTH*sizeof(int));
		bitmap24=bmp_buffer;
		load_bitmap_local(bmp,bpp);
		bitmap24=NULL;
	}
...
int load_bitmap_file(int* bmp,int bpp)
{
	...
	bmp_buffer=(int*)malloc(BMP_WIDTH*BMP_HEIGHT* sizeof(int)) ;
 ...
 int  show_bitmap(char *p, unsigned line_width, unsigned xres, unsigned yres, unsigned bpp)
{
	...
	bmp=(int*)malloc((size_t)(1920*1080*sizeof(int)+10));
	if(NULL==bmp) 
	{
 ...
 int  test_osd_display(void* para)
{
	...
	while (!exit)
	{
		...
		for (i=0;i<2;i++)
		{
			osd[i]=(osd_obj_t  *)create_osd_obj(i>0?mode:32,i);
